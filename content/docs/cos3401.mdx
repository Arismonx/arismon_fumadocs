...
## งานชุดที่ 2
#### ข้อที่ 1
ให้นักศึกษาหาขอบภาพที่สมบูรณ์ที่สุด ชัดเจนที่สุด ของสัญลักษณ์พิเศษบนศิลาหินในน้ํา (ออกแบบ
วิธีการเองอย่างเหมาะสม ให้ได้คําถามที่ดีที่สุดที่เป็นไปได้)
![1](/image/symbolics.jpg)
<Accordions type="single">
  <Accordion title="คำตอบ ข้อที่ 1"> 
...
def main():
        img_bgr = cv2.imread("./image/symbolics.jpg",1)
        
        #แยก B,G,R
        b,g,r = cv2.split(img_bgr)
        
        #ทำ norm แล้วทำ power gamma แต่ละสี
        #blue
        norm_b = normalize(b)
        new_b = power_gamma(0.5,norm_b)
        
        #green
        norm_g = normalize(g)
        new_g = power_gamma(0.5,norm_g)
        
        #red
        norm_r = normalize(r)
        new_r = power_gamma(0.5,norm_r)
        
        #merge
        img_merge = cv2.merge((new_b,new_g,new_r))
        
        #BGR2HSV
        img_hsv = cv2.cvtColor(img_merge, cv2.COLOR_BGR2HSV)
        # cv_show(img_hsv)
        ...
        #หาขอบ
        img_final = edge_operator_meth(mask,1)
        cv_show(img_final)
        cv2.imwrite('./out/img_edge.png',img_final)
...
if __name__ == '__main__':
    ...
    ![1](/image/img_edge.png)
  </Accordion>
</Accordions>

#### ข้อที่ 2
ให้นักศึกษาออกแบบโปรแกรมในการบูรณะภาพ โดยออกแบบอัลกอริธึมเอง (ตามความเหมาะสม) ใน
การปรับปรุงคุณภาพภาพระดับเทา เพื่อให้องค์ประกอบส่วนกล้ามเนื้อมีความชัดเจนมากยิ่งขึ้น โดยใช้
ภาพสีมาเป็นข้อมูลเบื้องต้นในการปรับปรุงภาพระดับเทา

![2](/image/body_color.jpg)
![2](/image/body_gray.jpg)
...
def main():
        ...
        img_gray = cv2.imread("./image/body_gray.jpg",0)
        img_gray[mask==255] = img_gray[mask == 255]*2.5
        
        cv_show(img_gray)
        cv2.imwrite('./out/final_img.png',img_gray)
